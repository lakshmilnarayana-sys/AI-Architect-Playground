...
## Likely cause
Failing downstream dependency, application bug, resource exhaustion, or an error-rate fault (`make fault SVC=<svc> MODE=high_error_rate`).

## Mitigation
1. Identify the failing dependency: check downstream health and error logs.
2. Clear the fault if injected: `make fault SVC=<svc> MODE=clear`.
3. Verify 5xx ratio returns below 5% after mitigation.
