...
- **Vector Store:** ChromaDB (Existing `vector_store/chroma`).
- **Graph Store:** Neo4j (Local instance via `NEO4J_URI`).
- **Embeddings:** Custom `StableEmbeddings` class (wrapping existing hash-based logic into LangChain's `Embeddings` interface).
- **LLM:** Integration via `LangChain-OpenAI` or similar (configured via `.env`).

## 4. Implementation Plan
1. **Wrappers:** Create `StableEmbeddings(Embeddings)` to maintain backward compatibility with existing data.
2. **Retrievers:** Initialize `Chroma` and `Neo4jGraph` as LangChain-compatible objects.
3. **Graph Nodes:**
    - `router_node`: Determines the path.
    - `vector_retrieval_node`: Fetches context from Chroma.
    - `graph_retrieval_node`: Fetches context from Neo4j (using Cypher).
    - `synthesizer_node`: Generates the final grounded answer.
4. **Comparison Tool:** A dedicated script to run the "10-query comparison" required by the guidelines, outputting a Markdown report.
